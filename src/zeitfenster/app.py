...
from zeitfenster.availability import fetch_and_compute
...
async def _regenerate(config: AppConfig, site_dir: Path) -> None:
    try:
...
async def _periodic_regeneration(config: AppConfig, site_dir: Path) -> None:
    while True:
        await asyncio.sleep(REGEN_INTERVAL_SECONDS)
        await _regenerate(config, site_dir)
...
async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
    ...
    task = asyncio.create_task(_periodic_regeneration(config, site_dir))
    try:
        yield
    finally:
        task.cancel()
...
async def book(
    ...
    ics_data = build_booking_ics(
        owner_email=config.email.owner,
        customer_name=name,
    ...
    asyncio.create_task(_regenerate(config, site_dir))
