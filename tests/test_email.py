...
@pytest.mark.asyncio
async def test_sends_auth_credentials_by_default(monkeypatch):
    monkeypatch.setenv("SMTP_HOST", "smtp.example.com")
    monkeypatch.setenv("SMTP_USER", "sender@example.com")
    monkeypatch.setenv("SMTP_PASSWORD", "secret")
    config = Email(owner="owner@example.com")
    ...
    assert send.call_args.kwargs["username"] == "sender@example.com"
...
@pytest.mark.asyncio
async def test_smtp_use_auth_false_disables_auth(monkeypatch):
    monkeypatch.setenv("SMTP_HOST", "smtp.example.com")
    monkeypatch.setenv("SMTP_USER", "sender@example.com")
    # SMTP_PASSWORD is deliberately left unset: with auth disabled, the
    # password property must never be accessed.
    config = Email(owner="owner@example.com", smtp_use_auth=False)
